...
def projected_coms(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> th.Tensor:
...
def projected_zmps(
    ...
    calculate_zmp_pos_w(env, asset_cfg, left_foot_sensor_cfg, right_foot_sensor_cfg)
...
def process_contact_data(
    ...
    c_env_flat = th.arange(N, device=device).unsqueeze(1).repeat(1, F).view(-1)  # [N * m]
    ...
    c_indices = th.repeat_interleave(c_idx_nonzero - cum_counts_nonzero + c_num_nonzero, c_num_nonzero) + th.arange(
        cum_counts_nonzero[-1], device=device
    )
    ...
    idx_in_env = th.arange(cum_counts[-1], device=device) - th.repeat_interleave(env_offsets, c_num_flat)
...
def compute_2d_convex_hull(p_points: th.Tensor, p_mask: th.Tensor, eps: float = 1e-6) -> th.Tensor:
    """
    ...
    for i in range(M):
        ...
        if valid_next.any():
            ...
            returned_to_start = curr_indices[valid_indices_a] == starting_indices[valid_indices_a]
    ...
    batch_indices = th.arange(N, device=p_points.device).unsqueeze(1).expand(N, M)[valid_hull_mask]  # [K_total]
...
def hull_point_signed_dist(hull_points: th.Tensor, hull_idx: th.Tensor, points: th.Tensor):
    """
    Args:
...
def is_point_in_2d_hull(points: th.Tensor, hull_points: th.Tensor, num_vertices: th.Tensor):
    """
    ...
    valid_angles = th.where(valid_mask, angles, th.tensor(float("nan"), device=points.device))
...
def compute_2d_hull_centroid(hull_points: th.Tensor, hull_idx: th.Tensor):
    ...
    centroid = th.full((N, 2), float("nan"), device=hull_points.device, dtype=th.float32)
    ...
    if mask_3.any():
        ...
        sorted_points = th.gather(points_3, dim=1, index=sorted_idx.unsqueeze(-1).expand(-1, -1, 2))
        sorted_mask = th.gather(mask_valid_3, dim=1, index=sorted_idx)

        # Remove NaNs
        x = th.where(sorted_mask, sorted_points[..., 0], th.zeros_like(sorted_points[..., 0]))
        y = th.where(sorted_mask, sorted_points[..., 1], th.zeros_like(sorted_points[..., 1]))
...
def compute_lin_ang_momentum(asset: Articulation, measured_frame: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
    """
    ...
    link_inertia_b = asset.data.default_inertia.clone().to(asset.device).view(N, -1, 3, 3)
...
def compute_rel_lin_ang_momentum(
    ...
    - angular_momentum: [N, 3] -> measured in the base frame    NOTE(ytcho):
        ...
        - Link velocities(self._root_physx_view.get_link_velocities()):
            ...
            link frame, not the principal axis of the link inertia.
    """
    link_com_pose_b = asset.root_physx_view.get_coms().clone().to(asset.device)[:, body_ids, :]
    ...
    N, _ = link_mass.shape
    # Inertia measured in the COM frame of each link
    link_inertia_b = asset.data.default_inertia.clone().to(asset.device).view(N, -1, 3, 3)[:, body_ids, :, :]
